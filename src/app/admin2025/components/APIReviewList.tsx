...
import { getAdminAPIs, approveAPI, type AdminAPI } from '@/services/admin-api'
...
export default function APIReviewList({ onAPIApproved }: APIReviewListProps) {
  ...
  const loadDraftAPIs = useCallback(async () => {
    try {
      ...
      console.log('🔄 [APIReviewList] 开始加载待审核API列表')
      
      const response = await getAdminAPIs({
        status: 'draft',
        page: 1,
      ...
      if (response.success) {
        ...
        console.log('✅ [APIReviewList] 待审核API列表加载成功:', response.data.length, '个API')
      } else {
  ...
  // 审核通过API
  const handleApprove = async (api: AdminAPI) => {
    if (approving) return
    
    try {
      setApproving(api.id)
      console.log('🔄 [APIReviewList] 开始审核通过 API:', api.name)
      
      const response = await approveAPI(api.id)
      
      if (response.success) {
        console.log('✅ [APIReviewList] API审核通过成功:', api.name)
      ...
      toast.error(error.message || '审核失败，请稍后重试')
    } finally {
      setApproving(null)
    }
  }
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <Badge 
                    variant={api.is_public ? "default" : "outline"} 
                    className="ml-2"
                  >
                    {api.is_public ? '公开' : '私有'}
                  </Badge>
