...
async function handler(request: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  try {
    ...
    const hasTrailingSlash =
      originalUrl.pathname.endsWith('/') && !originalUrl.pathname.endsWith('//')
    ...
    console.debug('🔗 原始URL:', request.url)
    console.debug('📍 解析路径:', path)
    console.debug('🔍 原始URL尾部斜杠:', hasTrailingSlash ? '✅有' : '❌无')
    console.debug('🧹 清理路径:', cleanPath)
    console.debug('🎯 目标URL:', fullUrl)
    console.debug('🔧 方法:', request.method)
    console.debug('⏰ 请求时间:', new Date().toISOString())
    console.debug('🌍 API基础URL:', API_BASE_URL)

    // 🔧 URL验证 - 确保不会导致重定向
    try {
      const parsedUrl = new URL(fullUrl)
      console.debug('✅ URL验证通过:')
      console.debug('  协议:', parsedUrl.protocol)
      console.debug('  主机:', parsedUrl.host)
      console.debug('  路径:', parsedUrl.pathname)
    ...
    request.headers.forEach((value, key) => {
      console.debug(
        `  ${key}: ${key.toLowerCase().includes('authorization') ? value.substring(0, 20) + '...' : value}`
      )
    ...
    if (['POST', 'PUT', 'PATCH'].includes(request.method) && !hasContentType) {
      ...
      console.debug('📝 [proxy] 添加默认Content-Type: application/json')
    }

    // 移除CORS头部（这些是给浏览器的，不应该发送到后端）
    console.debug('📤 转发请求头 (到后端):')
    Object.entries(headers).forEach(([key, value]) => {
      console.debug(
        `  ${key}: ${typeof value === 'string' && key.toLowerCase().includes('authorization') ? value.substring(0, 20) + '...' : value}`
      )
    })
    ...
    if (request.method === 'OPTIONS') {
      ...
      console.debug('🌐 这是浏览器发起的预检请求，用于检查跨域策略')
      console.debug('📋 预检请求头:')
      request.headers.forEach((value, key) => {
      ...
      console.debug('🎯 浏览器要求的方法:', requestMethod)
      console.debug('🎯 浏览器要求的头部:', requestHeaders)

      if (requestMethod === 'POST') {
        console.debug('✅ 预检请求确认要使用POST方法')
      } else if (requestMethod) {
        console.warn('⚠️  预检请求要求的方法不是POST:', requestMethod)
      }

      console.debug('📤 返回CORS响应头')
      console.groupEnd()
      console.groupEnd()
    ...
    if (['POST', 'PUT', 'PATCH'].includes(request.method)) {
      ...
      console.debug('📦 请求体长度:', body.length, '字符')
      console.debug('📦 请求体内容:')
      console.debug(body)
      // 尝试解析并美化JSON
      try {
        const jsonBody = JSON.parse(body)
        console.debug('📦 请求体 (格式化JSON):')
        console.debug(JSON.stringify(jsonBody, null, 2))
      } catch {
        console.debug('📦 请求体不是JSON格式')
      }
      ...
      console.debug('📦 无请求体 (方法:', request.method + ')')
    }
    ...
    console.debug('🚀 [proxy] 开始转发请求到后端')
    console.debug('🔧 [proxy] 转发方法:', request.method)
    console.debug('🔧 [proxy] 转发URL:', fullUrl)
    console.debug('🔧 [proxy] 转发配置:', {
      method: fetchConfig.method,
      hasBody: !!fetchConfig.body,
      bodyLength: fetchConfig.body
        ? typeof fetchConfig.body === 'string'
          ? fetchConfig.body.length
          : 'N/A'
        : 0,
    })
    ...
    if (request.method === 'POST') {
      console.debug('✅ [proxy] POST请求验证:')
      console.debug('  ✓ 方法确认: POST')
      console.debug('  ✓ URL确认:', fullUrl)
      console.debug(
        '  ✓ Content-Type:',
        headers['content-type'] || headers['Content-Type'] || '未设置'
      )
      console.debug(
        '  ✓ Authorization:',
        headers['authorization'] || headers['Authorization'] ? '已设置' : '未设置'
      )
      console.debug(
        '  ✓ Body长度:',
        fetchConfig.body
    ...
    // 发起代理请求
    console.debug('📡 [proxy] 即将发送到:', API_BASE_URL)
    console.debug('📡 [proxy] 完整请求详情:', {
      url: fullUrl,
      method: fetchConfig.method,
    ...
    if (response.redirected) {
      ...
      console.debug('✅ [proxy] 无重定向 - URL修复成功!')
    }
    ...
    // 验证响应
    console.debug('🔍 [proxy] 响应验证:')
    console.debug('  最终URL:', response.url)
    console.debug('  是否重定向:', response.redirected)
    console.debug('  响应类型:', response.type)
    console.debug('  原始方法确认:', request.method)

    console.debug(`⏱️ [proxy] 后端响应时间: ${fetchEndTime - fetchStartTime}ms`)
    ...
    console.debug('📊 响应状态:', response.status, response.statusText)
    console.debug('📋 响应头:')
    ...
    try {
      ...
      console.debug('📦 响应数据 (格式化JSON):')
      console.debug(JSON.stringify(jsonResponse, null, 2))
    } catch {
      console.debug('📦 响应数据不是JSON格式')
    }

    console.debug(`⏱️ 总处理时间: ${totalTime}ms`)
    console.groupEnd()
    ...
    responseHeaders.set(
      ...
      'Content-Type, Authorization, x-vola-key, x-vola-gateway'
    )

    console.debug('✅ [proxy] 成功转发响应到前端')
